...
const AxiosProvider = ({children})=> {
    ...
    const updateDirector = async(newInfo) => {
        let newDirectors = directors.map(c => c.id == newInfo.id ? newInfo : c)
    ...
    const updateMovie = async(newInfo) => {
        ...
        setMovies(newMovies)
        axios.put(`/api/directors/${newInfo.director_id}/movies/${newInfo.id}`, newInfo)
    }
    ...
    return (
        <AxiosContext.Provider value={{directors, movies, actors, updateDirector, newDirector, deleteDirector, updateActor, updateMovie,
    newActor, newMovie, deleteActor, deleteMovie}}>
            {children}
